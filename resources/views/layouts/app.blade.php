...
@php
  $navItems = [
    ['route' => 'wallet.index',   'pattern' => 'wallet.*',   'icon' => 'dashboard', 'label' => 'Dashboard'],
    ['route' => 'category.index', 'pattern' => 'category.*', 'icon' => 'category',  'label' => 'Kategori'],
  ];
  $isAdmin = auth()->user()?->role === 'admin';
@endphp

{{-- TOP BAR (mobile only) --}}
<header class="md:hidden flex items-center justify-between px-4 py-3 bg-forest-900 text-white sticky top-0 z-30">
  <a href="{{ route('wallet.index') }}" class="flex items-center gap-2">
    ...
    <span class="font-display font-extrabold text-base tracking-tight">o-wallet</span>
  </a>
...
<div class="flex min-h-screen">
  ...
  <aside class="hidden md:flex md:w-64 shrink-0 bg-forest-900 text-forest-100 flex-col py-6 px-5">
    <a href="{{ route('wallet.index') }}" class="flex items-center gap-2 px-2 mb-10">
    ...
    <nav class="flex flex-col gap-1">
      ...
      @if ($isAdmin)
        <a href="{{ route('dashboard.admin') }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors text-forest-200 hover:bg-forest-800 hover:text-white">
    ...
    <div class="mt-auto pt-6 border-t border-forest-800">
      <div class="flex items-center gap-3 px-2 mb-3">
        ...
        <div class="leading-tight min-w-0">
          ...
          <p class="text-xs text-forest-300 truncate">{{ auth()->user()->email }}</p>
        </div>
      </div>
...
<nav class="md:hidden fixed bottom-0 inset-x-0 z-30 bg-forest-900 border-t border-forest-800 flex items-stretch">
  @foreach ($navItems as $item)
    @php $active = request()->routeIs($item['pattern']); @endphp
    <a href="{{ route($item['route']) }}"
  ...
  @if ($isAdmin)
    <a href="{{ route('dashboard.admin') }}"
       class="flex-1 flex flex-col items-center justify-center gap-1 py-2.5 text-[11px] font-medium {{ request()->routeIs('dashboard.admin') || request()->routeIs('admin.*') ? 'text-white' : 'text-forest-400' }}">
